...
export default function Organisation(props) {
  const { logout, token, user, onDrawerToggle } = props;
  const { toggleLanguage } = React.useContext(AppThemeContext);
  ...
  const renderSelectGroup = (organisation) => {
    const items = organisation.suborganisations.map((option) => {
      return <MenuItem value={option._id}>{option.name}</MenuItem>;
    });
    return [
      <MenuItem value={organisation._id}>
