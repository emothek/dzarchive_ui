...
export default function Files(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    Axios.put(
      process.env.REACT_APP_HOSTNAME + `/file/${file.id}`,formValues,
    { withCredentials: true, headers: { Authorization: `Bearer ${token}` } })
  ...
  const createCategory=async (categorie)=>{
    ...
    await Axios.post(process.env.REACT_APP_HOSTNAME + "/category", {categorie}, {
      withCredentials: true,
      headers: { Authorization: `Bearer ${token}` },
    }).then( (res)=>{
  ...
  return (
    <Box sx={{ margin: "auto", overflow: "hidden" }}>
                  ...
                  .map((f, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{f.nArticle !== ""? f.nArticle : f.id}</TableCell>
                        <TableCell align="right">
      ...
      {openEf && <Dialog open={openEf} onClose={handleCloseEf}>
